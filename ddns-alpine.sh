...
# 检查依赖环境（不限制发行版，Alpine/CentOS/Arch 等均可）
check_env(){
    if ! command -v bash >/dev/null 2>&1; then
        echo -e "${Error}本脚本需要 bash，请先手动安装 bash 后重试。"
        exit 1
    fi
}
...
install_pkg() {
    if command -v apk >/dev/null 2>&1; then
        ...
        echo -e "${RED}未检测到支持的包管理器 (apk/apt-get/dnf/yum/pacman)，请手动安装: $*${NC}"
        return 1
    fi
}
...
check_curl() {
    ...
    if [ ${#missing[@]} -gt 0 ]; then
        echo -e "${YELLOW}未检测到 ${missing[*]}，正在安装...${NC}"
...
# 判断当前系统是否使用 systemd
has_systemd() {
    [ -d /run/systemd/system ] || { command -v systemctl >/dev/null 2>&1 && systemctl --version >/dev/null 2>&1; }
}

# 安装 cron 定时任务（systemd 不可用时兜底）
install_cron() {
    if [ -d /etc/cron.d ]; then
        cat > /etc/cron.d/ddns <<'EOF'
SHELL=/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
*/5 * * * * root /bin/bash /etc/DDNS/DDNS >> /var/log/ddns.log 2>&1
EOF
        chmod 644 /etc/cron.d/ddns
    else
        # 无 /etc/cron.d 目录时写入 root crontab
        local cron_line='*/5 * * * * /bin/bash /etc/DDNS/DDNS >> /var/log/ddns.log 2>&1'
        (crontab -l 2>/dev/null | grep -vF '/etc/DDNS/DDNS'; echo "$cron_line") | crontab -
    fi
    # 尝试启动 cron 守护进程
...
clear_cron() {
    ...
    if command -v crontab >/dev/null 2>&1; then
        crontab -l 2>/dev/null | grep -vF '/etc/DDNS/DDNS' | crontab - 2>/dev/null || true
    fi
}
...
check_ddns_status(){
    if has_systemd; then
        ...
        if [ -f /etc/cron.d/ddns ] || crontab -l 2>/dev/null | grep -qF '/etc/DDNS/DDNS'; then
            ddns_status="running"
        else
            ddns_status="dead"
        fi
...
go_ahead(){
  ...
  ${GREEN}0${NC}：退出脚本"
    ...
    case "$option" in
        ...
        9) echo -e "${Info}正在手动触发一次DDNS检查..."
           if has_systemd; then
               systemctl start ddns.service
           else
               /bin/bash /etc/DDNS/DDNS
           fi
           echo -e "${Info}已触发，请通过日志查看结果。"; sleep 2; main ;;
...
EOF
        ...
        echo -e "${Info}当前系统未使用 systemd，将使用 cron 作为调度方式。"
        install_cron
        echo -e "${Info}DDNS cron 任务已安装。将每5分钟检查一次IP变更。"
    fi
}
...
restart_ddns(){
    ...
    if has_systemd; then
        ...
        install_cron
        echo -e "${Info}DDNS cron 任务已启动。将每5分钟检查一次IP变更。"
    fi
}
